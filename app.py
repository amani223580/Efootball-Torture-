...
# ==========================================
# 1. ALGORITHM ZA NDANI (HELPERS) & SAFETY
# ==========================================
...
menu = [
    ...
    "🏆 Msimamo wa Knockouts & Waliofuzu",
    "🗓️ Ratiba Kamili ya Hatua (Fixtures)",
    "⚙️ Admin Hub"
]
choice = st.sidebar.selectbox("Chagua Menyu", menu)

# ==========================================
# 4. LOGIC YA KILA UKURASA (MENU ROUTING)
# ==========================================
...
if choice == "🏠 Home & Sheria":
    ...
    with col2:
        ...
        st.info("""
        * **Usajili wa Msimu:** Ligi inajaza wachezaji 16 kiwango cha juu, lakini inanyumbuka.
        * **Hatua ya Makundi:** Wachezaji wanagawanywa kwenye makundi kiotomatiki.
        * **Kufuzu Knockouts:** Viongozi wa makundi wanaenda Robo Fainali, Nusu Fainali hadi Fainali.
        """)
    ...
    if not all_leagues.empty:
        ...
        for g_name in ["Kundi A", "Kundi B", "Kundi C", "Kundi D"]:
            ...
            h_teams = [r[0] for r in c.fetchall()]
            c.execute("SELECT DISTINCT away FROM matches WHERE league_id = ? AND stage = ?", (l_id, g_name))
            a_teams = [r[0] for r in c.fetchall()]
            
            g_players = list(set(h_teams + a_teams))
            ...
            if g_players:
                ...
                for p in g_players:
                    if p not in res_dict:
                        res_dict[p] = {'Pts':0, 'GD':0, 'GF':0, 'GA':0, 'Pld':0, 'W':0, 'D':0, 'L':0}
        ...
        with col1:
            ...
            for g in ["Kundi A", "Kundi B", "Kundi C", "Kundi D"]:
                ...
                if len(res) >= 2:
                    ...
                    st.write(f"**{g}:** 1. {res[0][0]} ✅")
                else:
                    st.write(f"**{g}:** *Inasubiri...*")
                    
        with col2:
            st.markdown("### ⚡ Waliofuzu Nusu Fainali")
            for i in range(1, 5):
                w = get_match_winner(l_id, f"Robo Fainali {i}")
                if w: st.success(f"Mshindi Robo {i}: **{w}** ➡️ Semi")
    ...
    if not all_leagues.empty:
        ...
        if current_stage_row:
            st.metric(label="🚨 HATUA INAYOENDELEA SASA", value=f"{current_stage_row[0].upper()}")
        else:
    ...
    if not st.session_state.admin_logged_in:
        ...
        with tab1:
            ...
            if not players_df.empty:
                ...
                st.info("Hakuna wachezaji kwenye mfumo.")
            conn.close()
        ...
        with tab2:
            ...
            if active_row:
                ...
                if v_count >= 2:
                    if st.button("🚀 PANGA RATIBA YA MAKUNDI NA ANZISHA LIGI"):
